...
|-------|------|----------------|
| INTAKE | 项目接管分析师 | `project://intake` (source stays at user-provided path) |
| REQ | 需求分析师 | `state.paths.req` (`sop.yaml project_paths.req`) |
| PPT | PPT策划 | `state.paths.ppt_path`, initially `sop.yaml workspaces.ppt` (record actual path in SOP.md) |
| UI | 原型设计师 | `state.paths.ui` (`sop.yaml project_paths.ui`) |
| ARCH | 架构师 | `state.paths.arch` (`sop.yaml project_paths.arch`) |
| CODE | 研发工程师 | `state.paths.code` (`sop.yaml project_paths.code`) |
| TEST | 测试架构师 | `project://test` |
| DOCS | 文档专员 | `project://docs` |
| SOP | 编排官 | project root `SOP.md` |
...
- The default CODE URI uses **`project/`** (singular). Resolve it from config;
  do not reconstruct it from this convention.
